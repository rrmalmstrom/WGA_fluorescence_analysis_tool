...
2. **Extract** to a convenient location (e.g., `Documents/fluorescence_tool_simplified`)
...
2. **Navigate to Project Directory**
   ```bash
   cd path/to/fluorescence_tool_simplified
   ```
...
3. **Create Environment from File**
   ...
   This will:
   - Create a new environment called "fluorescence-tool"
...
```bash
conda env remove -n fluorescence-tool
```
...
```bash
python verification/end_to_end_verification.py
```
...
**Solutions**:
1. Ensure environment is activated: `conda activate fluorescence-tool`
2. Reinstall environment: `conda env remove -n fluorescence-tool` then recreate
...
pip install numpy==1.24 scipy==1.10 pandas==2.0 matplotlib==3.7 pytest
```
...
1. **Backup Current Installation**
   ```bash
   conda env export -n fluorescence-tool > my_environment_backup.yml
   ```
...
```bash
conda update conda
conda update anaconda  # if using Anaconda
```
...
#### Backup Environment
```bash
conda env export -n fluorescence-tool > fluorescence_tool_backup.yml
```

#### Restore Environment
```bash
conda env create -f fluorescence_tool_backup.yml
```
