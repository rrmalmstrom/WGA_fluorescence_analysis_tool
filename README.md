...
|------------|-----------------|----------|
| Python     | >=3.9, <3.13    | macOS / Linux (conda) |
| Python     | 3.11            | Windows (pip + venv) |
| numpy      | >=2.0, <3.0     | all |
| scipy      | >=1.13, <2.0    | all |
| pandas     | >=2.0, <3.0     | all |
| matplotlib | >=3.9, <4.0     | all |
| tkinter    | built into Python — no install needed | all |
| Windows deps | see [`requirements.txt`](requirements.txt) (auto-generated by CI) | Windows only |
...
├── environment.yml              # Conda environment specification (macOS / Linux)
